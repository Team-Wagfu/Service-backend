...
def Geolocation_validator(
    v: Dict[Literal["lat", "lng"], float],
) -> Dict[Literal["lat", "lng"], float]:
...
# other types
UsernameStr = Annotated[str, AfterValidator(username_validator)]
...
class IDs:
    ...
    EmergencyUserID = Annotated[str, AfterValidator(prefix("EME"))]

    DoctorID = Annotated[str, AfterValidator(prefix("DOC"))]

    AdminID = Annotated[str, AfterValidator(prefix("ADM"))]

    PharmaceuticalID = Annotated[str, AfterValidator(prefix("PHM"))]

    ClinicID = Annotated[str, AfterValidator(prefix("CLN"))]

    FacilityID = Annotated[str, AfterValidator(prefix("FAC"))]
